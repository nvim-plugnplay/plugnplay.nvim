...
function jobs.options()
   ...
   if jobs.cwd then options.cwd = jobs.cwd end

   if jobs.env then options.env = jobs.env end

   if jobs.detach then options.detach = jobs.detach end
...
jobs.shutdown = function(code, signal)
   if jobs.on_exit then jobs.on_exit(code, signal) end
   if jobs.on_stdout then jobs.stdout:read_stop() end
   if jobs.on_stderr then jobs.stderr:read_stop() end
   jobs.stop()
end
...
jobs.start = function()
   ...
   if jobs.on_stdout then vim.loop.read_start(jobs.stdout, vim.schedule_wrap(jobs.on_stdout)) end
   if jobs.on_stderr then vim.loop.read_start(jobs.stderr, vim.schedule_wrap(jobs.on_stderr)) end
end
